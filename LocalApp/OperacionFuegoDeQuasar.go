...
func main() {
  ...
  distances := [3]float32{}
  var palabrasKenobi []string
  var palabrasSkywalker []string
  var palabrasSato []string
  var x, y float32
  var mensajeDetectado string
  var posicionValida bool

  fmt.Println("---Fuego de Quasar---")
  fmt.Println("")

  fmt.Println("Ingrese distancia hacia satelite Kenobi: ")
  text, _ := reader.ReadString('\n')
  value,err:=strconv.ParseFloat(strings.TrimSpace(text), 32)
  if err != nil {
    log.Print(err)
  }
  distances[0] = float32(value)
  for {
    fmt.Println("Ingrese palabra hacia satelite Kenobi: ")
    text, _ = reader.ReadString('\n')
    palabra := strings.TrimSpace(text)
    if err != nil {
      log.Print(err)
    }
    palabrasKenobi = append(palabrasKenobi,palabra)
    fmt.Println("Desea agregar una palabra hacia satelite Kenobi? (NO='n',YES=any key)")
    resp, _ := reader.ReadString('\n')
    respuesta := strings.TrimSpace(resp)
    if err != nil {
      log.Print(err)
    }
    if (respuesta == "n"){
  ...
  fmt.Println("Ingrese distancia hacia satelite Skywalker: ")
  text, _ = reader.ReadString('\n')
  value,err=strconv.ParseFloat(strings.TrimSpace(text), 32)
  if err != nil {
    log.Print(err)
  }
  distances[1] = float32(value)
  for {
    fmt.Println("Ingrese palabra hacia satelite Skywalker: ")
    text, _ = reader.ReadString('\n')
    palabra := strings.TrimSpace(text)
    if err != nil {
      log.Print(err)
    }
    palabrasSkywalker = append(palabrasSkywalker,palabra)
    fmt.Println("Desea agregar una palabra hacia satelite Skywalker? (NO='n',YES=any key)")
    resp, _ := reader.ReadString('\n')
    respuesta := strings.TrimSpace(resp)
    if err != nil {
      log.Print(err)
  ...
  fmt.Println("Ingrese distancia hacia satelite Sato: ")
  text, _ = reader.ReadString('\n')
  value,err=strconv.ParseFloat(strings.TrimSpace(text), 32)
  if err != nil {
    log.Print(err)
  }
  distances[2] = float32(value)
  for {
    fmt.Println("Ingrese palabra hacia satelite Sato: ")
    text, _ = reader.ReadString('\n')
    palabra := strings.TrimSpace(text)
    if err != nil {
      log.Print(err)
    }
    palabrasSato = append(palabrasSato,palabra)
    fmt.Println("Desea agregar una palabra hacia satelite Sato? (NO='n',YES=any key)")
    resp, _ := reader.ReadString('\n')
    respuesta := strings.TrimSpace(resp)
    if err != nil {
      log.Print(err)
  ...
  fmt.Println("----------------------------------------------------------------------------------")
  x, y = GetLocation(distances[0],distances[1],distances[2])
  posicionValida = validarPosicionEncontrada(distances[0],A1,B1,distances[1],A2,B2,distances[2] ,A3,B3,x,y)
  if (posicionValida) {
    fmt.Println("La posicion de la nave enemiga es: ["+fmt.Sprint(x)+","+fmt.Sprint(y)+"]")
    mensajeDetectado = GetMessage(palabrasKenobi,palabrasSkywalker,palabrasSato)
    fmt.Println("El mensaje detectado es: ")
    fmt.Println(mensajeDetectado)
  } else {
    fmt.Println("La posicion calculada de la nave enemiga es invalida para los datos recibidos")
  }
  fmt.Println("----------------------------------------------------------------------------------")
}
...
func validarPosicionEncontrada(D1 float32, A1 float32, B1 float32, D2 float32, A2 float32, B2 float32, D3 float32, A3 float32, B3 float32, x float32, y float32) (bool) {
